...
export function createApiRouter(config: ApiRouterConfig): Router {
  ...
  router.post(
    '/session',
    (req: Request, res: Response<CreateSessionResponse | ErrorResponse>) => {
      try {
        ...
        logger.info(
          { sessionId: session.sessionId, type: finalType },
          'Session created',
        );
  ...
  router.post(
    '/message',
    (req: Request, res: Response<SendMessageResponse | ErrorResponse>) => {
      try {
        ...
        if (!sessionId || !content) {
          res
            .status(400)
            .json({ error: 'Session ID and content are required' });
          return;
        }
  ...
  router.get(
    '/messages',
    (req: Request, res: Response<GetMessagesResponse | ErrorResponse>) => {
      try {
        const { sessionId, since } = req.query as {
          sessionId?: string;
          since?: string;
        };
  ...
  router.delete(
    '/session/:sessionId',
    (req: Request, res: Response<void | ErrorResponse>) => {
      try {
